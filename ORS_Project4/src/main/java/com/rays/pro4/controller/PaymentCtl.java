...
public class PaymentCtl extends BaseCtl{
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(request.getParameter("type"))) {
			...
			request.setAttribute("type","First type must contains alphabet only");
			pass = false;
		}
		...
		if (DataValidator.isNull(request.getParameter("amount"))) {
			...
			request.setAttribute("amount", "Last type must contains number only");
			pass = false;
		}
...
//		log.debug("PaymentCtl Method doGet Started");
...
//		// get model
		...
		ServletUtility.forward(ORSView.PAYMENT_VIEW, request, response);
...
System.out.println("dopodt ....");
		...
		if ( OP_UPDATE.equalsIgnoreCase(op)) {
			...
			bean.getId();
				...
				/*
				 * ServletUtility.setBean(bean, request);
				 * ServletUtility.setSuccessMessage("Payment is successfully saved", request);
				 */
