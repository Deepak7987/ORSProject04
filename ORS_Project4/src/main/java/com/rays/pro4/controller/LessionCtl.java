...
public class LessionCtl extends BaseCtl{
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(request.getParameter("subject"))) {
			...
			request.setAttribute("subject", "Last Name must contains alphabest only");
			pass = false;
		}
...
//		log.debug("LessionCtl Method doGet Started");
...
//		// get model
		...
		ServletUtility.forward(ORSView.LESSION_VIEW, request, response);
...
System.out.println("dopodt ....");
		...
		if ( OP_UPDATE.equalsIgnoreCase(op)) {
			...
			bean.getId();
				...
				/*
				 * ServletUtility.setBean(bean, request);
				 * ServletUtility.setSuccessMessage("Lession is successfully saved", request);
				 */
			...
			System.out.println(" U ctl D p 5555555");
			 ...
			 bean = (LessionBean) populateBean(request);
			try {
				model.delete(bean);
