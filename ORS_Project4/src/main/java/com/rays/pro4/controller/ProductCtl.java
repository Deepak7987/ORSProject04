...
public class ProductCtl extends BaseCtl{
	...
	protected boolean validate(HttpServletRequest request) {
		 ...
		 * false; } if (DataValidator.isNull(request.getParameter("Producttype"))) {
		 * request.setAttribute("Producttype", PropertyReader.getValue("error.require",
		 * " Producttype")); pass = false; } else if
		 * (!DataValidator.isName(request.getParameter("Producttype"))) {
		 * request.setAttribute("Producttype", "Last dob must contains number only"); pass
		 * = false; }
		 */
	...
	protected BaseBean populateBean(HttpServletRequest request) {
		...
		bean.setSize(DataUtility.getString(request.getParameter("size")));
		bean.setPrize(DataUtility.getString(request.getParameter("prize")));
...
//		// get model
		...
		ServletUtility.forward(ORSView.PRODUCT_VIEW, request, response);
...
System.out.println("dopodt ....");
		...
		if ( OP_UPDATE.equalsIgnoreCase(op)) {
			...
			bean.getId();
				...
				/*
				 * ServletUtility.setBean(bean, request);
				 * ServletUtility.setSuccessMessage("Product is successfully saved", request);
				 */
